...
export const TimelineBlock = () => {
    ...
    const controlRef = useRef<HTMLDivElement>(null); // для анимации мобильного блока

    const animateChange = (callback: () => void) => {
        callback();

        if (isMobile && controlRef.current) {
            // Мобильная анимация: подъём блока с пагинацией и каруселью
            const tl = gsap.timeline();
            tl.set(controlRef.current, {opacity: 0, y: 20});
            tl.to(controlRef.current, {opacity: 1, y: 0, duration: 0.5, ease: "power2.out"});
        }

        if (!isMobile && carouselRef.current) {
            // Десктоп: обычная анимация затухания карусели
            const tl = gsap.timeline();
    ...
    return (
        <div className={s.container}>
            <Title/>
            {!isMobile && <div className={s.circleAbsolute}>
                <Circle
                segments={segments}
                total={totalSegments}
